...
static volatile bool _transferOnGoing = false;

bool is_transfer_ongoing()
{
	return _transferOnGoing;
}
...
void led_render() {
  if(wr_buf_p != 0 || hdma_tim2_ch2_ch4.State != HAL_DMA_STATE_READY) {
  ...
  _transferOnGoing = true;
  HAL_TIM_PWM_Start_DMA(&htim2, TIM_CHANNEL_2, (uint32_t *)wr_buf, WR_BUF_LEN);
